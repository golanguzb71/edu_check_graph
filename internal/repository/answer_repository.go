...
func (r *AnswerRepository) Create(answer *model.Answer) error {
	_, err := r.db.Exec("INSERT INTO answers (is_true, question_id, answer_field) VALUES (?, ?, ?)",
...
func (r *AnswerRepository) Get(id int) (*model.Answer, error) {
	row := r.db.QueryRow("SELECT id, is_true, question_id, answer_field, created_at, updated_at FROM answers WHERE id = ?", id)
	answer := &model.Answer{}
...
func (r *AnswerRepository) Update(answer *model.Answer) error {
	_, err := r.db.Exec("UPDATE answers SET is_true = ?, question_id = ?, answer_field = ?, updated_at = ? WHERE id = ?",
...
// Delete removes an answer from the database.
func (r *AnswerRepository) Delete(id int) error {
	_, err := r.db.Exec("DELETE FROM answers WHERE id = ?", id)
	if err != nil {
